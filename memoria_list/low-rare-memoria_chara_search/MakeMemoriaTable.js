...
function makeTable() {
    ...
    for (let i = 0; i < lowRareMemoriaJsonCopy.length; i++) {
        ...
        // サムネ画像要素の追加
        let img = document.createElement("img");
        img.src = "../../images/low-rare-memoria/low-rare-memoria_" + lowRareMemoriaJsonCopy[i]["id"] + ".webp"
...
// 初期化
let lowRareMemoriaJsonCopy = JSON.parse(JSON.stringify(lowRareMemoriaJson));
for (let i = 0; i < lowRareMemoriaJsonCopy.length; i++) {
    lowRareMemoriaJsonCopy[i]["tr"] = "";
}
makeTable();
let selectionArray = [];
let onlymode = false
