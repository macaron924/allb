...
function getItemName(item) {
    ...
    switch (true) {
        ...
        case item.startsWith('gacha-ticket'):
            ...
            link = `gacha_info/ticket/index.html?index=${index}`;
            break;
        ...
        case item.startsWith('exchange_costume'):
            ...
            link = `exchange_info/content/index.html?index=${item}`;
            break;
...
function getGachaData(gachaSelection) {
    let str = gachaSelection.substr(0, 6);
    let list;
    for (let i = 0; i < gachaLimitedList.length; i++) {
        if (gachaLimitedList[i]['str'] == str) list = gachaLimitedList[i]['ref'];
    }
