...
// 役割とスキル名をもとに情報を取得
function getSkillNameFromName(skillJson, yakuwari, skillName) {
...
// 頭文字と補助スキル名をもとに情報を取得
function getHojoSkillNameFromName(hojoJson, skillName) {
    let hojoGroup;
...
const urls = [
    ...
    { dataName: "gachaTicketJson", urlName: `${path}data/gacha-ticket_data.json` },
    { dataName: "gachaDaily11", urlName: `${path}data/gacha_data/gacha-daily11.json` },
    { dataName: "gachaFree", urlName: `${path}data/gacha_data/gacha-free.json` },
    { dataName: "exchangeDataJson", urlName: `${path}data/exchange_data.json` },
    { dataName: "skillJson", urlName: `${path}data/memoria_skill.json` },
    { dataName: "hojoJson", urlName: `${path}data/memoria_hojo.json` },
]
...
Promise.all(fetches)
    .then(result => {
        ...
        for (let i in resultsObjects.gachaTicketJson) {
            const thisTicket = resultsObjects.gachaTicketJson[i];
            let name = thisTicket["name"];
            let index = thisTicket["index"];
            let date = thisTicket["lineup"]["else"][0]["date"];
            let caption = thisTicket["lineup"]["else"][0]["caption"];
            let content = [];
            for (let j in thisTicket["lineup"]["else"]) {
                content = content.concat(thisTicket["lineup"]["else"][j]["content"]);
            }
        ...
        let freeContentList = [];
        for (let i in resultsObjects.gachaFree) {
            const thisGachaFree = resultsObjects.gachaFree[i];
            let start = thisGachaFree["start"];
            for (let j in thisGachaFree["gacha"]) {
                let index = thisGachaFree["gacha"][j]["index"];
                let name = thisGachaFree["gacha"][j]["name"];
                let content = [];
                let pu = thisGachaFree["gacha"][j]["lineup"]["pu"];
                let other = thisGachaFree["gacha"][j]["lineup"]["else"];
                for (let k in pu) {
        ...
        for (let i in resultsObjects.gachaDaily11) {
            const thisGachaDaily11 = resultsObjects.gachaDaily11[i];
            let start = thisGachaDaily11["start"];
            for (let j in thisGachaDaily11["gacha"]) {
                let index = thisGachaDaily11["gacha"][j]["index"];
                let name = thisGachaDaily11["gacha"][j]["name"];
                let content = [];
                let pu = thisGachaDaily11["gacha"][j]["lineup"]["pu"];
                let other = thisGachaDaily11["gacha"][j]["lineup"]["else"];
                for (let k in pu) {
                    content = content.concat(pu[k]["content"]);
                }
                content = content.concat(other["content"]);
        ...
        for (let i in resultsObjects.exchangeDataJson) {
            const thisExchangeDataJson = resultsObjects.exchangeDataJson[i];
            for (let j in thisExchangeDataJson) {
                let name = thisExchangeDataJson[j]["name"];
                let index = thisExchangeDataJson[j]["index"];
                let get = thisExchangeDataJson[j]["get"];
                let content = [];
                for (let k in thisExchangeDataJson[j]["lineup"]["memoria"]) {
                    content = content.concat(thisExchangeDataJson[j]["lineup"]["memoria"][k]["content"]);
                }
                exchangeContentList.push({ name: name, index: index, get: get, content: content })
            }
        }
        
        document.getElementById("memoria_list").appendChild(getMemoriaList(resultsObjects, Object.keys(resultsObjects.memoriaJson)));
        ...
        if (memoriaSelection != null) {
            ...
            const thisSkill = thisMemoria["skill"];
            for (let i in thisSkill) {
                ...
                vshDetail.innerText = getSkillNameFromName(resultsObjects.skillJson, yakuwari, vsh);
                ...
                lmDetail.innerText = getSkillNameFromName(resultsObjects.skillJson, yakuwari, lm);
                ...
                hojoDetail.innerText = getHojoSkillNameFromName(resultsObjects.hojoJson, hojo);
            ...
            document.getElementById("result_name").innerHTML = `${thisMemoria["name"]}`;
