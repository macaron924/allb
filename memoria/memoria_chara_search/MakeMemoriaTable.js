...
// テーブル作成
function makeTable(memoriaJsonCopy, mode) {
...
const urls = [
    ...
    { dataName: "lrmemoriaJson", urlName: `${path}data/low-rare-memoria.json` }
]
...
Promise.all(fetches)
    .then(result => {
        ...
        if (mode == "lowrare") {
            memoriaJsonCopy = JSON.parse(JSON.stringify(resultsObjects.lrmemoriaJson));
            document.getElementById("low").classList.remove("active");
            document.getElementById("high").classList.add("active");
        } else {
            memoriaJsonCopy = JSON.parse(JSON.stringify(resultsObjects.memoriaJson));
        }
        for (let id in memoriaJsonCopy) {
            memoriaJsonCopy[id]["tr"] = "";
        }
        makeCharaList(resultsObjects);
        makeTable(memoriaJsonCopy, mode);
